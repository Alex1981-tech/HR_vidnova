from __future__ import annotations

from collections import defaultdict
from typing import Any

from django.core.exceptions import ImproperlyConfigured
from django.core.management.base import BaseCommand, CommandError
from django.db import transaction
from django.db.models import Count, Q
from django.utils import timezone

from apps.employees.models import Clinic, Department, DepartmentLevel, Employee
from apps.integrations.models import PeopleForceEntity, PeopleForceImportRun
from apps.integrations.peopleforce.client import PeopleForceClient
from apps.integrations.peopleforce.importer import DEFAULT_CLINIC_CODE, clean, ext_id, make_code, name_of, payload_hash, trim


class Command(BaseCommand):
    help = "Import PeopleForce departments only."

    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true", help="Fetch and count without writing departments.")

    def handle(self, *args, **options):
        counters: dict[str, int] = defaultdict(int)
        run = PeopleForceImportRun.objects.create(
            status=PeopleForceImportRun.Status.RUNNING,
            options={
                "command": "sync_peopleforce_departments",
                "dry_run": options["dry_run"],
            },
        )
        try:
            client = PeopleForceClient()
            if options["dry_run"]:
                self._sync(client, run, counters, dry_run=True)
            else:
                with transaction.atomic():
                    self._sync(client, run, counters, dry_run=False)
            run.status = PeopleForceImportRun.Status.DRY_RUN if options["dry_run"] else PeopleForceImportRun.Status.COMPLETED
            run.finished_at = timezone.now()
            run.counters = dict(counters)
            run.save(update_fields=["status", "finished_at", "counters", "updated_at"])
        except ImproperlyConfigured as exc:
            run.status = PeopleForceImportRun.Status.FAILED
            run.finished_at = timezone.now()
            run.error_message = str(exc)
            run.counters = dict(counters)
            run.save(update_fields=["status", "finished_at", "error_message", "counters", "updated_at"])
            raise CommandError(str(exc)) from exc
        except Exception as exc:
            run.status = PeopleForceImportRun.Status.FAILED
            run.finished_at = timezone.now()
            run.error_message = str(exc)
            run.counters = dict(counters)
            run.save(update_fields=["status", "finished_at", "error_message", "counters", "updated_at"])
            raise

        self.stdout.write(self.style.SUCCESS(f"PeopleForce departments import run #{run.id}: {run.status}"))
        for key, value in sorted(counters.items()):
            self.stdout.write(f"{key}: {value}")

    def _sync(self, client: PeopleForceClient, run: PeopleForceImportRun, counters: dict[str, int], *, dry_run: bool) -> None:
        rows = client.list_all("/departments")
        counters["fetched_departments"] = len(rows)
        clinic = self._default_clinic(dry_run=dry_run)
        for payload in rows:
            self._upsert_department(payload, run, counters, clinic=clinic, dry_run=dry_run)
        for payload in rows:
            self._resolve_links(payload, counters, dry_run=dry_run)

    def _default_clinic(self, *, dry_run: bool) -> Clinic | None:
        if dry_run:
            return None
        clinic = (
            Clinic.objects.exclude(code=DEFAULT_CLINIC_CODE)
            .filter(is_active=True)
            .annotate(active_employees=Count("employees", filter=Q(employees__status=Employee.Status.ACTIVE)))
            .order_by("-active_employees", "name")
            .first()
        )
        if clinic:
            return clinic
        clinic = Clinic.objects.exclude(code=DEFAULT_CLINIC_CODE).order_by("name").first()
        if clinic:
            return clinic
        clinic, _ = Clinic.objects.get_or_create(
            code=DEFAULT_CLINIC_CODE,
            defaults={"name": "PeopleForce import", "external_peopleforce_id": "", "is_active": True},
        )
        return clinic

    def _upsert_department(
        self,
        payload: dict[str, Any],
        run: PeopleForceImportRun,
        counters: dict[str, int],
        *,
        clinic: Clinic | None,
        dry_run: bool,
    ) -> Department | None:
        name = trim(clean(payload.get("name")), 160)
        external_id = ext_id(payload)
        if not name or not clinic:
            counters["departments_skipped"] += 1
            return None
        counters["departments_seen"] += 1
        if dry_run:
            return None
        self._store_entity("departments", external_id, "/departments", payload, run)
        level = self._department_level_from_payload(payload, counters, dry_run=dry_run)
        code = make_code("pf-", name, external_id, 60)
        department = Department.objects.filter(external_peopleforce_id=external_id).first() if external_id else None
        if department is None:
            department, created = Department.objects.get_or_create(
                clinic=clinic,
                name=name,
                defaults={"code": code, "external_peopleforce_id": external_id, "level": level, "is_active": True},
            )
        else:
            created = False
            department.name = name
            department.code = code
            department.clinic = clinic
            department.level = level
            department.is_active = True
            department.save(update_fields=["name", "code", "clinic", "level", "is_active", "updated_at"])
        if external_id and not department.external_peopleforce_id:
            department.external_peopleforce_id = external_id
            department.save(update_fields=["external_peopleforce_id", "updated_at"])
        counters[f"departments_{'created' if created else 'updated'}"] += 1
        return department

    def _resolve_links(self, payload: dict[str, Any], counters: dict[str, int], *, dry_run: bool) -> None:
        if dry_run:
            return
        department = Department.objects.filter(external_peopleforce_id=ext_id(payload)).first()
        if not department:
            return
        update_fields = []
        parent_id = clean(payload.get("parent_id"))
        if parent_id:
            parent = Department.objects.filter(external_peopleforce_id=parent_id).first()
            if parent and department.parent_id != parent.id:
                department.parent = parent
                update_fields.append("parent")
        manager_id = clean(payload.get("manager_id"))
        if manager_id:
            manager = Employee.objects.filter(legacy_peopleforce_id=manager_id).first()
            if manager and department.manager_id != manager.id:
                department.manager = manager
                update_fields.append("manager")
        level = self._department_level_from_payload(payload, counters, dry_run=False)
        if level and department.level_id != level.id:
            department.level = level
            update_fields.append("level")
        if update_fields:
            department.save(update_fields=[*update_fields, "updated_at"])
            counters["departments_links_updated"] += 1

    def _department_level_from_payload(
        self,
        payload: dict[str, Any],
        counters: dict[str, int],
        *,
        dry_run: bool,
    ) -> DepartmentLevel | None:
        level_payload = payload.get("department_level")
        external_id = ext_id(level_payload or {}) if isinstance(level_payload, dict) else clean(payload.get("department_level_id"))
        if not external_id and not isinstance(level_payload, dict):
            return None
        name = trim(name_of(level_payload) if isinstance(level_payload, dict) else "", 160) or f"Level {external_id}"
        if not name:
            return None
        counters["department_levels_seen"] += 1
        if dry_run:
            return None
        level = DepartmentLevel.objects.filter(external_peopleforce_id=external_id).first() if external_id else None
        if level is None:
            level, created = DepartmentLevel.objects.get_or_create(
                name=name,
                defaults={
                    "external_peopleforce_id": external_id,
                    "color": trim(clean((level_payload or {}).get("color") or (level_payload or {}).get("hex_color")), 16) or "#94a3b8",
                    "is_active": True,
                },
            )
        else:
            created = False
            level.name = name
            level.is_active = True
            level.save(update_fields=["name", "is_active", "updated_at"])
        if external_id and not level.external_peopleforce_id:
            level.external_peopleforce_id = external_id
            level.save(update_fields=["external_peopleforce_id", "updated_at"])
        counters[f"department_levels_{'created' if created else 'updated'}"] += 1
        return level

    def _store_entity(
        self,
        entity_type: str,
        external_id: str,
        endpoint: str,
        payload: dict[str, Any],
        run: PeopleForceImportRun,
    ) -> None:
        if not external_id:
            return
        PeopleForceEntity.objects.update_or_create(
            entity_type=entity_type,
            external_id=external_id,
            defaults={
                "endpoint": endpoint,
                "payload": payload,
                "payload_hash": payload_hash(payload),
                "fetched_at": timezone.now(),
                "last_run": run,
            },
        )
